...
sagnik1.dam@adamasuniversity.ac.in}
\and
...
sambit.biswas@adamasuniversity.ac.in}
\and
...
mayukh.saha@adamasuniversity.ac.in}
\and
...
hrittima.sen@adamasuniversity.ac.in}
\vspace{0.15in}
\and
...
arpan.das@adamasuniversity.ac.in}
\and
...
pushpita.debnath@adamasuniversity.ac.in}
\and
\IEEEauthorblockN{Mr. Bibhas Das}
\IEEEauthorblockA{\textit{Coordinator / Mentor}\\
\textit{Dept. of Computer Science \& Eng.}\\
\textit{School of Eng. \& Tech. (SOET)}\\
\textit{Adamas University}, Kolkata, India\\
bibhas.das@adamasuniversity.ac.in}
}
...
\begin{IEEEkeywords}
Applicant Tracking Systems, Hybrid Vector Search, Information Retrieval, Natural Language Processing, LLM Evaluation, Skill Gap Analysis.
\end{IEEEkeywords}

\section{Introduction}
The exponential expansion of digital recruitment portals and remote hiring channels has led to an unprecedented volume of job applications submitted to corporate portals daily. Human Resource (HR) departments rely heavily on Applicant Tracking Systems (ATS) to filter, parse, score, and rank incoming candidate profiles against complex Job Descriptions (JDs). Despite widespread adoption, traditional ATS platforms exhibit fundamental architectural failure modes that severely impair recruitment accuracy and hiring efficiency.
...
\begin{enumerate}
    \item \textbf{Semantic Inelasticity}: Lexical keyword matching algorithms (e.g., Okapi BM25) rely strictly on exact string tokens. Candidates who describe technical competencies using alternative nomenclature or contextual synonyms (e.g., listing ``Relational Database Design'' instead of ``PostgreSQL'') are heavily penalized and filtered out.
...
\subsection{Dense Vector Embeddings and RAG Architectures}
...
\begin{table}[htbp]
\caption{Taxonomic Comparison of ATS Retrieval Paradigms}
\label{tab:taxonomy}
\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Paradigm} & \textbf{Semantic} & \textbf{Hard Rules} & \textbf{Layout} & \textbf{Efficiency} \\
\midrule
Lexical (BM25) & Low & High & None & $O(1)$ \\
Dense Vector & High & Low & None & $O(\log N)$ \\
Zero-Shot LLM & High & Medium & Low & $O(N)$ \\
\textbf{PrepGenius} & \textbf{V. High} & \textbf{High} & \textbf{High} & \textbf{Multi-Stage} \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
The PrepGenius architecture is structured into five decoupled operational layers: File Intake and Magic Byte Inspection, Structural ATS Layout Auditing, Bulk Boundary Segmentation, Multi-Stage Hybrid Vector Matching, and Multi-Factor Score Fusion.

\subsection{File Intake and Magic Byte Disambiguation}
...
\end{equation}
where $N_e$ denotes critical layout errors (table box-drawing characters), $N_w$ denotes layout warnings (multi-column text wrapping), and $N_i$ denotes informational notices (repeating headers). Penalty coefficients are set to $\lambda_e = 15$, $\lambda_w = 5$, and $\lambda_i = 1$.
...
\begin{table}[htbp]
\caption{Multi-Factor Scoring Weights}
\label{tab:weights}
\begin{center}
\begin{tabular}{llr}
\toprule
...
\caption{PrepGenius Hybrid Retrieval Algorithm}
\label{alg:hybrid}
\begin{algorithmic}[1]
...
\section{Experimental Evaluation and Benchmark Results}
...
The benchmark dataset comprised 150 structured candidate resumes across four engineering domains (Software Engineering, Data Science, Product Management, and Technical Sales), matched against 20 complex technical job descriptions. Relevance labels ($\text{rel}_i \in \{0, 1, 2, 3\}$) were assigned through blind evaluation by three independent recruitment professionals.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model Architecture} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} & \textbf{NDCG@10} \\
\midrule
...
\section{Discussion, Ethical Safeguards and Hiring Fairness}

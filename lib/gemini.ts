...
export async function generateText(prompt: string, systemInstruction?: string, maxOutputTokens = 1024, json = true, responseSchema?: GeminiSchema) {
  const body = {
    ...(systemInstruction ? { systemInstruction: { parts: [{ text: systemInstruction }] } } : {}),
    contents: [{ role: "user", parts: [{ text: prompt }] }],
    generationConfig: {
      ...(json ? { responseMimeType: "application/json", ...(responseSchema ? { responseSchema } : {}) } : {}),
      maxOutputTokens,
    },
  };
...
export async function generateJson<T>(prompt: string, systemInstruction: string, responseSchema?: GeminiSchema): Promise<T> {
  let lastError: unknown;
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const text = await generateText(prompt, `${systemInstruction}${retry}`, 2048, true, responseSchema);
